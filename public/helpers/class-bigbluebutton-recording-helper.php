	...
	public function order_recordings( $order = '', $order_by = '' ) {
		if ( '' !== $order && '' !== $order_by ) {
			...
			usort(
				...
				$result = strcasecmp( $self->get_recording_field( $first, $field ), $self->get_recording_field( $second, $field ) );
			...
			$self      = $this;
			// var_dump($this->recordings); die;

			usort(
				$this->recordings,
				function ( $first, $second ) use ( $direction, $field, $self ) {
					$a = $self->get_recording_field( $first, $field );
					$b = $self->get_recording_field( $second, $field );

					// Use spaceship operator for numeric or string comparison
					if ( is_numeric( $a ) && is_numeric( $b ) ) {
						$result = $a <=> $b; // numeric
					} else {
						$result = strcasecmp( (string) $a, (string) $b ); // string
					}
