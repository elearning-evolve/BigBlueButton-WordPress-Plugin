...
private function join_meeting( $return_url, $room_id, $username, $entry_code, $viewer_code, $wait_for_mod ) {
	...
	if ( $entry_code == $viewer_code && 'true' == $wait_for_mod ) {
		if ( Bigbluebutton_Api::is_meeting_running( $room_id ) ) {
			$this->request_post_on_iframe_join( $join_url );
		} else {
			$query = array(
			...
			$this->request_post_on_iframe_join( add_query_arg( $query, $return_url ) );
		}
	} else {
		$this->request_post_on_iframe_join( $join_url );
	}
	...
	/**
	 * If join IFrame then create a post request else simply redirect
	 *
	 * @since   3.0.0
	 *
	 * @param   String  $join_url       Join URL for the meeting.
	 */
private function request_post_on_iframe_join( $join_url ) {
	$j_url_parts = parse_url( esc_url_raw( $join_url ) );

	if ( isset( $j_url_parts['query'] ) ) {
		parse_str( $j_url_parts['query'], $params );
		if ( isset( $params['bbb_room_join'] ) ) {
			
			$req_url = $j_url_parts['scheme'] . '://' . $j_url_parts['host'] . $j_url_parts['path'];

			$org_response = wp_remote_get(
				esc_url_raw( base64_decode( $params['bbb_room_join'] ) ),
				array(
					'timeout' => 60,
				)
			);
			...
			if ( is_wp_error( $org_response ) ) {
				Bigbluebuttonpro_Helper::reload_with_js();
			}

			$response = wp_remote_retrieve_body( $org_response );
			$error    = Bigbluebuttonpro_Helper::is_xml( $response );

			if ( $error ) {
				Bigbluebuttonpro_Helper::reload_with_js();
			} else {
				if ( isset( $org_response['http_response']->get_response_object()->url ) ) {
					$session_join_url = sanitize_url( $org_response['http_response']->get_response_object()->url );
					$join_url         = add_query_arg(
						array(
							'roomId'        => sanitize_text_field( $params['room_id'] ),
							'bbb_room_join' => $session_join_url,
						),
						$req_url
					);
				}
	...
	// Normally redirect to BBB meeting join URL
		wp_redirect( esc_url_raw( $join_url ) );
		exit;
}
}
